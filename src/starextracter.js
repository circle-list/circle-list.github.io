    ...
    setTimeout(function() {
        ...
        if(src !== undefined) {
            if(swiper.isEnd === true) {
                console_log('[1/3] Generating image list Finished')
                success()
            } else {
                images.push(src)
                console_log('[1/3] Generating image list (' + swiper.activeIndex + ' / ' + swiper.slides.length + ')')
                swiper.slideNext()
                genImg()
            }
        } else {
